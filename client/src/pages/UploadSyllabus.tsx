...
const UploadSyllabus = () => {
  ...
  async function submit_material(e: ChangeEvent<HTMLFormElement>) {
    ...
    if (!subject) {
      ...
      toast("Please select subject", { style: { background: "#333", color: "#fff" } , icon: "❌", position: "top-right" });
      return;
    }
    if (!type) {
      // toast.error("Please choose material type.");
      toast("Please chhose material type.", { style: { background: "#333", color: "#fff" } , icon: "❌", position: "top-right" });
      return;
    }
    if (!name) {
    
      toast('File description/name is required', { style: { background: "#333", color: "#fff" } , icon: "❌", position: "top-right" });
      return;
    }
    if (!doc) {
     
      toast("Please select file.", { style: { background: "#333", color: "#fff" } , icon: "❌", position: "top-right" });
      return;
    }
    ...
    <Layout>
      ...
      >
        ...
        >
          ...
          <h4 className="text-2xl font-semibold mb-8">Upload material here</h4>
          ...
          >
            ...
            {user?.subject.map((item: ApplicationType["subject"]) => (
              <option key={item.code} value={item.code} className="capitalize">
                {item.name}
              </option>
            ))}
          </select>
          ...
          <label htmlFor="desc" className="text-lg font-medium block">
            {" "}
            Description :
          </label>
          <input
            ...
            type="text"
            id="desc"
            placeholder="eg. previous exam papers"
          />
          <FileUploader
                  ...
                  {data?.map(
                    (item: {
                      ...
                      >
                        ...
                        >
                          <p className="underline">{item.desc}</p>
                          <small className="px-2 text-red-500 text-sm no-underline">
                            ({" "}
                            {
                              user?.subject?.find(
                                (a: { code: string }) => a.code === item.subject
                              ).name
                            }
                            )
                          </small>
